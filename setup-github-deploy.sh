...
echo ""
echo "Production database (pick one):"
echo "  1) Cloud SQL: create Secret Manager secret DATABASE_URL (postgres URL with ?host=/cloudsql/PROJECT:REGION:INSTANCE)"
echo "     and set GCLOUD_SQL_INSTANCE=PROJECT:REGION:INSTANCE"
echo "  2) Hosted Postgres (Neon etc.): same DATABASE_URL secret, set MOUNT_DATABASE_URL=true"
echo "  Grant Cloud Run SA roles/cloudsql.client on the project if using Cloud SQL."
echo ""
